    ...
    if (this_piece.classList.contains('movable') === true){
        ...
        if (player.pieces[pieceNr][1] === true){//if on Field
            if (player.playerOnStart){//change playerOnStart if player is moved away from his starting Field
                switch (player.name){
                    case "B":{if (piecePosition === 0){player.playerOnStart = false;}break;}
                    case "R":{if (piecePosition === 10){player.playerOnStart = false;}break;}
                    case "Y":{if (piecePosition === 20){player.playerOnStart = false;}break;}
                    case "G":{if (piecePosition === 30){player.playerOnStart = false;}break;}
                }
            }
            let rangeBegin_start = 4;
            switch (player.name){
                case "Y":rangeBegin_start+=10;break;
                case "G":rangeBegin_start+=20;break;
                case "B":rangeBegin_start+=30;break;
            }
            ...
            switch (player.name){
                case "B":{newPos = 0;break;}
                case "R":{newPos = 10;break;}
                case "Y":{newPos = 20;break;}
                case "G":{newPos = 30;break;}
            }
            killPiece('F'+newPos, player);
            this_piece.style.gridArea = 'F'+newPos;
        }
...
function anyPieceOutside(player) { //returns true if any piece is on any field (excluded: start & finish Fields)
    let out = false;
    for (let i = 0; i < 4; i++) {if (player.pieces[i][1])out = true;}
    return out;
}
function anyPieceHome(player) { //returns true if any piece is in his Home (Finish Fields)
    let out = false;
    for (let i = 0; i < 4; i++) {if (player.pieces[i][3])out = true;}
    return out;
}
function checkGameOver(player){
    let houseFull = true;
    for (let i = 0; i < 4; i++) {if (!player.pieces[i][3]) houseFull = false;}
    if (houseFull) {
    ...
    for (const originalPosKey in originalPos_temp) {
        if (originalPos_temp[originalPosKey] === position){
            if (originalPosKey < 4){
                ...
                if (position === 'F0'){playerBlue.playerOnStart = false;}
            }
                ...
                if (position === 'F10'){playerRed.playerOnStart = false;}
            }
                ...
                if (position === 'F20'){playerYellow.playerOnStart = false;}
            }
            else{
                ...
                if (position === 'F30'){playerGreen.playerOnStart = false;}
            }
            break;
        }
    }
}
/*Move Player Manually*/function mp(playerName, pieceNr, number) {document.getElementById('P' + playerName + '_' + pieceNr).classList.add('movable');switch (playerName){case 'B':{diceB[1] = number;clickOnPlayer(playerBlue, diceB, pieceNr);break;}case 'R':{diceR[1] = number;clickOnPlayer(playerRed, diceR, pieceNr);break;}case 'Y':{diceY[1] = number;clickOnPlayer(playerYellow, diceY, pieceNr);break;}case 'G':{diceG[1] = number;clickOnPlayer(playerGreen, diceG, pieceNr);break;}}}